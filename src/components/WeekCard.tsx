...
import { TrainingWeek, TrainingDay, formatDateString, getDayStatus } from '../utils/dateUtils';
...
const DayDisplay: React.FC<{ day: TrainingDay }> = ({ day }) => {
  ...
  return (
    <div className={`p-3 rounded-md ${bgColor} ${textColor} mb-2 transition-all duration-300 ease-in-out`}>
      <p className="font-semibold text-sm">{formatDateString(day.date, 'EEE, MMM d')}</p>
...
const WeekCard: React.FC<WeekCardProps> = ({ weekData, isCurrentWeekView }) => {
  return (
    <div className={`mb-8 p-4 rounded-lg shadow-xl ${isCurrentWeekView ? 'bg-gray-800' : 'bg-gray-750 border border-gray-700'}`}>
      <h3 className="text-2xl font-bold mb-1 text-indigo-400">Week {weekData.week}</h3>
      <p className="text-sm text-gray-400 mb-1">Starting: {formatDateString(weekData.startDate)}</p>
      <p className="text-md font-semibold text-teal-400 mb-4">{weekData.focus}</p>
